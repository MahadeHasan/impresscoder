document.addEventListener("DOMContentLoaded", function(){
  ...
  if(topbarSelector !== null){
    ...
    document.querySelector(':root').style.setProperty('--impresscoder-navbar-height', navbarSelector.offsetHeight + 'px');
  }
  if(topbarSelector !== null){
    topbarrHeight = topbarSelector.offsetHeight;
    document.querySelector(':root').style.setProperty('--impresscoder-topbar-height', topbarSelector.offsetHeight + 'px');
  }
  ...
  if(document.querySelectorAll(navSelector).length > 0){
    ...
    window.addEventListener('scroll', function() {
        if (window.scrollY > topbarrHeight) {
          ...
          document.querySelector(':root').style.setProperty('--impresscoder-header-height', headerHeight + 'px');
        } else {
          ...
          document.querySelector(':root').style.setProperty('--impresscoder-header-height', 0);
        } 
    });
  }
