...
final class Design_Options {
	...
	public function __construct() {
		...
		// Add HTML classes.
		add_filter( 'impresscoder_html_classes', array( $this, 'html_classes' ) );
	...
	public function editor_custom_color_variables() {
		...
		wp_enqueue_script(
			'impresscoder-editor-dark-mode-support',
			get_template_directory_uri() . '/assets/js/editor-dark-mode-support.js',
			array( 'impresscoder-dark-mode-support-toggle' ),
			'1.0.0',
			true
		);
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'tt1-dark-mode', $url, array( 'impresscoder-style' ), wp_get_theme()->get( 'Version' ) ); // @phpstan-ignore-line. Version is always a string.
	}
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script(
			'impresscoder-customize-controls',
			get_template_directory_uri() . '/assets/js/customize.js',
			array( 'customize-base', 'customize-controls', 'underscore', 'jquery', 'impresscoder-customize-helpers' ),
			'1.0.0',
			true
		);
	}
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors', 'impresscoder' );
		}
		...
		$description  = '<p>';
		$description .= sprintf(
			/* translators: %s: Impresscoder support article URL. */
			__( 'Dark Mode is a device setting. If a visitor to your site requests it, your site will be shown with a dark background and light text. <a href="%s">Learn more about Dark Mode.</a>', 'impresscoder' ),
			esc_url( __( 'https://wordpress.org/support/article/impresscoder/#dark-mode-support', 'impresscoder' ) )
		);
		$description .= '</p>';
		$description .= '<p>' . __( 'Dark Mode can also be turned on and off with a button that you can find in the bottom corner of the page.', 'impresscoder' ) . '</p>';
	...
	public function admin_body_classes( $classes ) {
		...
		if ( $current_screen->is_block_editor() ) {
			...
			if ( $should_respect_color_scheme && Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
				$classes .= ' impresscoder-supports-dark-theme';
			}
		}
	...
	public function the_html( $attrs = array() ) {
		...
		printf(
			...
			esc_html__( 'Dark Mode: %s', 'impresscoder' ),
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'impresscoder' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'impresscoder' ); ?>';
			}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'Impresscoder uses LocalStorage when Dark Mode support is enabled.', 'impresscoder' ) . '</p>'
				. '<strong class="privacy-policy-tutorial">' . __( 'Suggested text:', 'impresscoder' ) . '</strong> '
				. __( 'This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', 'impresscoder' );
		wp_add_privacy_policy_content( __( 'Impresscoder', 'impresscoder' ), wp_kses_post( wpautop( $content, false ) ) );
	}
