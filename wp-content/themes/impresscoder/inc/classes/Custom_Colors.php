...
class Custom_Colors {
	...
	public function custom_color_variables() {
		if ( 'd1e4dd' !== strtolower( get_theme_mod( 'background_color', 'D1E4DD' ) ) ) {
			wp_add_inline_style( 'impresscoder-style', $this->generate_custom_color_variables() );
		}
		$colors = impresscoder_get_default_colors_global();
		$colors_css =  [];
		$prefix = 'impresscoder-';
		foreach ($colors as $color) {
			$color_mod = get_theme_mod( 'impresscoder_'.$color['id'], $color['value'] );
		...
		if(!empty(array_filter($colors_css))){
			wp_add_inline_style( 'impresscoder-style', ':root{'.implode(' ',$colors_css).'}' );
		}
	...
	public function editor_custom_color_variables() {
		wp_enqueue_style(
			'impresscoder-customizer-preview',
		...
		$background_color = get_theme_mod( 'background_color', 'D1E4DD' );
		if ( 'd1e4dd' !== strtolower( $background_color ) ) {
			wp_add_inline_style( 'impresscoder-customizer-preview', $this->generate_custom_color_variables( 'editor' ) );
		}
	}
